...
class PasswordGeneratorApp():
   def __init__(self, master):
      ...
      #messagebox.showinfo("Successfully copy to clipboard!")
   ...
   def generate_and_display_password(self):
      try:
         ...
         self.copy_button.config(state='normal') # Enable the copy to clipboard button

      except ValueError as ve:
               messagebox.showerror("Entry Error", str(ve))
               self.generated_password_var.set("") #Clean the field
               self.copy_button.config(state='disabled')
